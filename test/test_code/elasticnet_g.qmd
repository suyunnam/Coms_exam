...
weather_recipe <-
  ...
  step_rm(year, site, matches("Jan|Feb|Mar|Apr|Nov|Dec")) %>%    # Remove data from Nov-Apr data when cottons are not growing
...
It can be different whenever I run again due to randomization process (seeds, v-fold)
RMSE and R2 results might not agree each other
...
final_fit %>%
  collect_predictions()
```
Why 210 observations? the number of test set data points were 210
...
```
How does metrics on test compare to metrics on train?  mostly, (but not always) training set has better result than test set. Because we made the model with the training set. Still, we need to report RMSE and R2 of the test set. Sometimes we can report both.
